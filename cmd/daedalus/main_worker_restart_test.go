...
func TestWorkerRestartUsesCLIConfig(t *testing.T) {
	...
	// defused per-subtest by noDaemonSpawn. Empty workerType: these pin the
	// CLI-config choice, so the restart stays untyped like the real bare
	// invocation when no -t is passed.
...
// daemonArgv waits for the spawned daemon child to report its argv in the
// worker's log (the child runs asynchronously; workerStart returns as soon
// as it is detached) and returns the line split back into arguments.
func daemonArgv(t *testing.T, logFile string) []string {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
	for {
		line, err := os.ReadFile(logFile)
		if err != nil {
			t.Fatalf("read daemon log: %v", err)
		}
		if trimmed := strings.TrimSpace(string(line)); trimmed != "" {
			return strings.Split(trimmed, "\x1f")
		}
		if time.Now().After(deadline) {
			t.Fatalf("daemon log %s stayed empty — the child never reported its argv", logFile)
		}
		time.Sleep(5 * time.Millisecond)
	}
}

// TestWorkerStartReexecType pins the run type's survival across the daemon
// re-exec: workerStart appends -t <type> to the child's argv exactly when a
// type was given, so the daemon comes up with the invoking command's poller
// selection rather than silently untyped. The observable is the child's real
// argv, printed into the daemon log by the reexecDumpArgsEnv branch of
// TestMain (the child is this test binary; its stdout is the daemon log).
func TestWorkerStartReexecType(t *testing.T) {
	// reexecedArgs runs the given start/restart entrypoint against a fresh
	// throwaway daemon dir and returns the config path plus the daemon
	// child's argv, read back from the log the child's stdout lands in.
	reexecedArgs := func(t *testing.T, start func(cfg config.Config, cfgPath string) error) (cfgPath string, argv []string) {
		t.Helper()
		useDaemonDir(t)
		...
		out := captureStdout(t, func() {
			if err := start(cfg, cfgPath); err != nil {
				t.Errorf("start: %v", err)
			}
		})
		if !strings.Contains(out, "worker started") {
			t.Fatalf("start output %q should report a started worker", out)
		}
		_, logFile, _ := daemonPaths("daedalus")
		return cfgPath, daemonArgv(t, logFile)
	}

	t.Run("an explicit type travels with the re-exec", func(t *testing.T) {
		cfgPath, got := reexecedArgs(t, func(cfg config.Config, p string) error {
			return workerStart(cfg, p, "dev")
		})
		want := []string{"worker", "foreground", "-c", cfgPath, "-t", "dev"}
		if !reflect.DeepEqual(got, want) {
			t.Errorf("daemon argv = %v, want %v", got, want)
		}
	})

	t.Run("no type revives the daemon untyped", func(t *testing.T) {
		cfgPath, got := reexecedArgs(t, func(cfg config.Config, p string) error {
			return workerStart(cfg, p, "")
		})
		want := []string{"worker", "foreground", "-c", cfgPath}
		if !reflect.DeepEqual(got, want) {
			t.Errorf("daemon argv = %v, want %v (no -t)", got, want)
		}
	})

	t.Run("a bare restart forwards the invoking type", func(t *testing.T) {
		cfgPath, got := reexecedArgs(t, func(cfg config.Config, p string) error {
			return workerRestart(cfg, p, "test")
		})
		want := []string{"worker", "foreground", "-c", cfgPath, "-t", "test"}
		if !reflect.DeepEqual(got, want) {
			t.Errorf("daemon argv = %v, want %v", got, want)
		}
	})
...
// TestWorkerRestartNamedRevivesUntyped pins the record-driven restart's
// untyped revival: `worker restart <name>` has no -t on its own command
// line (parseFlags rejects one outright) and passes none down either, so
// the daemon it brings back polls both queues regardless of how it was
// originally started — a run config is never persisted.
